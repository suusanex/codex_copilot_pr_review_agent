...
- `.github/agents/local-reviewer.agent.md`
  - `model: "gpt-5.5"` を必須指定し、PR差分と収集済み文脈からローカルCodexレビュー結果を作る読み取り専用agent。
...
- `scripts/install-codex-copilot-pr-review-agent-local.cs`
  - `.apm/agents/*.agent.md` を原本として、APM導入後に対象リポジトリへ `.codex/config.toml` と `.codex/agents/*.toml`、skill配布物を反映するインストーラ。

## 導入

対象リポジトリへAPMで導入後、まずインストーラで `.codex` とskill配布物を反映する。
...
標準ワークフローでは、`local-reviewer` でローカルCodexレビューを作成し、`review-planner` で統合修正計画を作成し、`spark-implementer` で計画範囲を実装する。
